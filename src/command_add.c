...
int			is_error_add(t_main *main, t_array_string *lex_str, char op)
{
	if (op == (char)0x54)
		return (0);
...
void		build_args_add(t_main *main, t_array_string *lex_str, int *args, int i)
{
...
void		command_add(t_main *main, t_array_string *lex_str)
{
	...
	if (ret == 0 || is_error_add(main, lex_str, op))
		return ;

	build_args_add(main, lex_str, args, i + 1);
	add_bcode(&main->bcode, new_bcode(4, op, args));
	main->pc += 5;
}
