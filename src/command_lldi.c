...
int			is_error_lldi(t_main *main, t_array_string *lex_str, char op)
{
...
void		inc_pc_lldi(t_main *main, char op)
{
	if (op == (char)0x54)
		main->pc += 5;
...
void		build_args_lldi(t_main *main, t_array_string *lex_str, int *args, int i)
{
...
void		command_lldi(t_main *main, t_array_string *lex_str)
{
	...
	if (ret == 0 || is_error_lldi(main, lex_str, op))
		return ;

	build_args_lldi(main, lex_str, args, i + 1);
	add_bcode(&main->bcode, new_bcode(14, op, args));
	inc_pc_lldi(main, op);
}
