...
int			is_error_xor(t_main *main, t_array_string *lex_str, char op)
{
	if (op == (char)0x54 ||
		op == (char)0xa4 ||
		op == (char)0xb4 ||
...
void		inc_pc_and_or_xor(t_main *main, char op)
{
	if (op == (char)0x54)
		main->pc += 5;
	else if (op == (char)0xa4)
		main->pc += 11;
	else if (op == (char)0xb4 || op == (char)0xe4)
		main->pc += 9;
	else
		main->pc += 7;
}

void		build_args_xor(t_main *main, t_array_string *lex_str, int *args, int i)
{
...
void		command_xor(t_main *main, t_array_string *lex_str)
{
	...
	if (ret == 0 || is_error_xor(main, lex_str, op))
		return ;

	build_args_xor(main, lex_str, args, i + 1);
	add_bcode(&main->bcode, new_bcode(8, op, args));
